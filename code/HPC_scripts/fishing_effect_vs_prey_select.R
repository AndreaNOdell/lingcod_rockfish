library("SimDesign", lib.loc = "../R_Packages/R4.2.2")
library("purrr", lib.loc = "../R_Packages/R4.2.2")
library("deSolve", lib.loc = "../R_Packages/R4.2.2")
library("parallel", lib.loc = "../R_Packages/R4.2.2")
...
fishing = seq(0, 0.4, length.out = 20)

fishing_base_selec <- mcmapply(get_pop_ts, f = fishing,
                                    MoreArgs = list(rockfish, lingcod, nsim = nsims, corr = 0, autocorr_L = 0.23, 
                                                    autocorr_R = autocorr_R, cv = cv, tf = tf, 
                                                    rec.yr = rec.year, mpa.yr = mpa.year, hist.f = hist.f, hist.by = hist.by, 
                                                    b = bycatch, quant95 = 0.29, handling = handl, min.selectivity = TRUE, 
                                                    min.age.consumed = 4, rockfish.prop = rock.prop, yelloweye.prop = yellow.prop),
                               SIMPLIFY = FALSE, mc.cores = parallel::detectCores()-1)

save(fishing_base_selec, file = "fishing_data0724/fishing_base_selec.Rdata")
save(fishing, file = "fishing_data0724/fishing.Rdata")



rock.prop = 0.1 # extreme prey selectivity
yellow.prop = 0.5 # extreme prey selectivity

fishing_extreme_selec <- mcmapply(get_pop_ts, f = fishing,
                               MoreArgs = list(rockfish, lingcod, nsim = nsims, corr = 0, autocorr_L = 0.23, 
                                               autocorr_R = autocorr_R, cv = cv, tf = tf, 
                                               rec.yr = rec.year, mpa.yr = mpa.year, hist.f = hist.f, hist.by = hist.by, 
                                               b = bycatch, quant95 = 0.29, handling = handl, min.selectivity = TRUE, 
...
save(fishing_extreme_selec, file = "fishing_data0724/fishing_extreme_selec.Rdata")
